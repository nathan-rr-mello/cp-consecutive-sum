...
void intercala(int** vetor, int tam_vetor) {
    ...
    while (i < tam_esquerda && j < tam_vetor) {
        if ((*vetor)[i] <= (*vetor)[j]) {
            result[k++] = (*vetor)[i++];
        } else {
            result[k++] = (*vetor)[j++];
...
int main(int argc, char **argv) {
    ...
    int* vetor;
    int tam_vetor = VETOR_SIZE; // defino tamanho inicial do vetor
    int pai;
    ...
    if (my_rank != 0) {
        ...
        vetor = (int*) malloc(tam_vetor * sizeof(int)); // aloco vetor com tamanho recebido
        pai = status.MPI_SOURCE;
        MPI_Recv(&vetor[0], tam_vetor, MPI_INT, pai, MPI_ANY_TAG, MPI_COMM_WORLD, &status); // não sou a raiz, tenho pai
    } else {
        // inicializa(&vetor, tam_vetor);      // sou a raiz e portanto gero o vetor - ordem reversa
        vetor = inicializa(tam_vetor);
    }
